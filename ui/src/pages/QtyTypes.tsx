...
const API_BASE = "/api/qty-types";

type Row = { id: number; name: string | null };

export default function QtyTypes() {
  ...
  async function onDeleteSelected() {
    ...
    const ok = confirm(`Delete ${selectedIds.length} qty type(s)?`);
    if (!ok) return;
  ...
  async function onDeleteOne(id: number) {
    const ok = confirm("Delete this qty type?");
    if (!ok) return;
  ...
  async function onSave() {
    if (!form.name.trim()) {
      alert("Quantity type is required");
      return;
    }
  ...
  return (
    <div style={page}>
      <div style={headerRow}>
        <div>
          <div style={title}>Quantity Types</div>
          <div style={subtitle}>Add / Edit / Find unit type (qty)</div>
        </div>

        <div style={searchRow}>
          <input
            style={searchInput}
            placeholder="Find (qty type)…"
            value={q}
      ...
      <div style={card}>
        ...
        <div style={formGridOne}>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={label}>Unit Type (Qty) *</label>
            <input
              style={inputFull}
              value={form.name}
              onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))}
              placeholder="Enter unit type (qty)"
            />
          </div>
      ...
      <div style={{ ...card, flex: 1, display: "flex", flexDirection: "column", width: "98%" }}>
        <div style={cardHead}>
          <div style={{ width: "100%" }}>
            <div style={cardTitle}>Quantity Types List</div>
        ...
        <div style={tableWrap}>
          <table style={table}>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td style={emptyTd} colSpan={3}>No qty types found.</td>
                </tr>
              ) : (
                rows.map((r) => (
...
/* same styles you already use in Clients/DeliveryPlaces/PaymentTypes/Flags */
