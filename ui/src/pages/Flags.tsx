...
export default function Flags() {
  ...
  async function onDeleteSelected() {
    ...
    const ok = confirm(`Delete ${selectedIds.length} flag(s)?`);
    if (!ok) return;
  ...
  async function onDeleteOne(id: number) {
    const ok = confirm("Delete this flag?");
    if (!ok) return;
  ...
  return (
    <div style={page}>
      <div style={headerRow}>
        <div>
          <div style={title}>Flag</div>
          <div style={subtitle}>Add / Edit / Find flags</div>
        </div>
      ...
      <div style={{ ...card, flex: 1, display: "flex", flexDirection: "column", width: "98%" }}>
        <div style={cardHead}>
          <div style={{ width: "100%" }}>
            <div style={cardTitle}>Flag List</div>
        ...
        <div style={tableWrap}>
          <table style={table}>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td style={emptyTd} colSpan={3}>No flags found.</td>
                </tr>
              ) : (
                rows.map((r) => (
