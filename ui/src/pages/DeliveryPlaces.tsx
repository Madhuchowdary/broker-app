...
export default function DeliveryPlaces() {
  ...
  async function onDeleteSelected() {
    ...
    const ok = confirm(`Delete ${selectedIds.length} delivery place(s)?`);
    if (!ok) return;
    ...
    <div style={page}>
      ...
      <div style={headerRow}>
        <div>
          ...
          <div style={subtitle}>Add / Edit / Find delivery places</div>
        </div>
        ...
        <div style={tableWrap}>
          <table style={table}>
            ...
            <tbody>
              {places.length === 0 ? (
                <tr>
                  <td style={emptyTd} colSpan={3}>
                    No delivery places found.
                  </td>
                </tr>
              ) : (
                places.map((p) => (
                  <tr key={p.id} style={rowStyle(form.id === p.id)}>
                    <td style={td}>
                      <input
                        type="checkbox"
                    ...
                    <td style={{ ...td, textAlign: "right" }}>
                      <button style={btnSmall} onClick={() => onModify(p)}>
                        Modify
                      </button>
                      <button style={btnDangerSmall} onClick={() => onDeleteOne(p.id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
...
/* styles (same as your client style) */
...
function rowStyle(active: boolean) {
  return { background: active ? "#eef2ff" : "white" } as React.CSSProperties;
}
