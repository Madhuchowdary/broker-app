...
const API_BASE = "/api/rate-per-unit";

type Row = { id: number; name: string | null };

export default function RatePerUnit() {
  ...
  async function onDeleteSelected() {
    ...
    const ok = confirm(`Delete ${selectedIds.length} rate(s)?`);
    if (!ok) return;
  ...
  async function onSave() {
    if (!form.name.trim()) {
      alert("Rate per unit is required");
      return;
    }
  ...
  return (
    <div style={page}>
      <div style={headerRow}>
        <div>
          <div style={title}>Rate Per Unit</div>
          <div style={subtitle}>Add / Edit / Find unit type (rate)</div>
        </div>

        <div style={searchRow}>
          <input
            style={searchInput}
            placeholder="Find (rate)…"
            value={q}
      ...
      <div style={card}>
        ...
        <div style={formGridOne}>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={label}>Unit Type (Rate) *</label>
            <input
              style={inputFull}
              value={form.name}
              onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))}
              placeholder="Enter unit type (rate)"
            />
          </div>
      ...
      <div style={{ ...card, flex: 1, display: "flex", flexDirection: "column", width: "98%" }}>
        <div style={cardHead}>
          <div style={{ width: "100%" }}>
            <div style={cardTitle}>Rate List</div>
        ...
        <div style={tableWrap}>
          <table style={table}>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td style={emptyTd} colSpan={3}>No rate records found.</td>
                </tr>
              ) : (
                rows.map((r) => (
...
/* styles: same as your other screens (copy/paste from QtyTypes/Flags) */
